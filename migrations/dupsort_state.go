...
var zstd = Migration{
	...
	Up: func(tx ethdb.Database, datadir string, OnLoadCommit etl.LoadCommitHandler) error {
		...
		cd128_minus1, err := gozstd.NewCDictLevel(dict128, -1)
		if err != nil {
			panic(err)
		}
		defer cd128_minus1.Release()

		cd128_0, err := gozstd.NewCDictLevel(dict128, 0)
		if err != nil {
			panic(err)
		}
		defer cd128_0.Release()
		...
		cd128_2, err := gozstd.NewCDictLevel(dict128, 2)
		if err != nil {
			panic(err)
		}
		defer cd128_2.Release()
		...
		cd128_4, err := gozstd.NewCDictLevel(dict128, 4)
		if err != nil {
			panic(err)
		}
		defer cd128_4.Release()
		...
		for k, v, err := c.First(); k != nil; k, v, err = c.Next() {
			...
			case <-logEvery.C:
				...
				log.Info("Progress 8", "blockNum", blockNum, "before", common.StorageSize(total),
					...
					"128_minus1", fmt.Sprintf("%.2f", totalf/float64(total128_minus1)), "d_minus1", d_minus1,
